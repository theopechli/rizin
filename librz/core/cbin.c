...
static RzCoreStringType get_string_type(const ut8 *buf, ut64 len) {
	rz_return_val_if_fail(buf && len, RZ_CORE_STRING_KIND_UNKNOWN);
	ut64 needle = 0;
	int rc, i;
	RzCoreStringType str_type = RZ_CORE_STRING_KIND_UNKNOWN;
	...
	while (needle < len) {
		...
		if (needle + rc + 2 < len &&
			...
			str_type = RZ_CORE_STRING_KIND_WIDE16;
		} else {
			str_type = RZ_CORE_STRING_KIND_ASCII;
		}
		for (i = 0; needle < len; i += rc) {
			RzRune r;
			if (str_type == RZ_CORE_STRING_KIND_WIDE16) {
				if (needle + 1 < len) {
				...
				if (rc > 1) {
					str_type = RZ_CORE_STRING_KIND_UTF8;
				}
			}
 ...
 */
RZ_OWN RZ_API RzCoreString *rz_core_string_information(RzCore *core, const char *block, int len, RzCoreStringType type) {
	...
	cstring->size = len;
	if (type != RZ_CORE_STRING_KIND_UNKNOWN) {
		cstring->type = type;
	} else {
		cstring->type = get_string_type(core->block, len);
	}
