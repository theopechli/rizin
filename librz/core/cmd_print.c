...
static void print_json_string(RzCore *core, const char *block, int len, RzCoreStringType type) {
	RzCoreString *cstring = rz_core_string_information(core, block, len, type);
	if (!cstring) {
		return;
	}
