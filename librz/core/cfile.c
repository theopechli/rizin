...
RZ_API void rz_core_sysenv_begin(RzCore *core) {
	...
	if (desc && desc->name) {
		...
		rz_sys_setenv("RZ_SIZE", rz_strf(s, "%" PFMT64d, rz_io_desc_size(desc)));
	}
	rz_sys_setenv("RZ_OFFSET", rz_strf(s, "%" PFMT64d, core->offset));
	rz_sys_setenv("RZ_XOFFSET", rz_strf(s, "0x%08" PFMT64x, core->offset));
	rz_sys_setenv("RZ_ENDIAN", core->rasm->big_endian ? "big" : "little");
	rz_sys_setenv("RZ_BSIZE", rz_strf(s, "%d", core->blocksize));
